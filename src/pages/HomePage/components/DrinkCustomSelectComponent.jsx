...
function CustomSelect({ options, placeholder, setSelectedDrink }) {
  ...
  return (
    <Root
      ...
      style={{width:'100%'}}
    >
      <Toggle {...getButtonProps()} style={{ '--color': value, width:'calc(100% - 13px)' }}>
        {value ?? <span className="placeholder">{placeholder ?? ' '}</span>}
      </Toggle>
      <Listbox {...getListboxProps()} className={listboxVisible ? '' : ''} >
        {options.map((option) => (
...
export default function CustomSelectComponent(props) {
  ...
  return <CustomSelect placeholder="Select a drink..." options={options} setSelectedDrink={props.setSelectedDrink}  />;
}
